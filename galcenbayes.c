/*
 * galcenbayes.c
 * Perform a Bayesian analysis for given survey parameters, as described in
 *  Chennamangalam & Lorimer (2014). In addition to lognormal luminosity
 *  function, also performs analysis for power law.
 *
 * Usage: galcenbayes smin nu beta N
 ...
 *  
 * Example: galcenbayes 0.050 4.85 -0.8 100000 > survey.df
 *  Use plotpdf.py to plot the contents of survey.df
 *
...
#define NMAX    100000
#define FMIN    0.001   /* 0.1% */
#define FMAX    0.999   /* 99.9% */
int main(int argc, char **argv) 
{
  ...
  if (argc<5)
  {
    puts("Usage: galcenbayes smin nu beta N");
    exit(0);
  }
  smin=atof(argv[1]);
  nu=atof(argv[2]);
